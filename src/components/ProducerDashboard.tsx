...
// keep your original ProducerDashboard but with tiny change: mutation status check uses .status string
...
const CATEGORIES = ["Vegetables","Fruits","Dairy","Bakery","Meat","Grains","Other"];
...
export default function ProducerDashboard() {
  ...
  const { data: producerStock = [], isRefetching: producerRefetching } = useQuery({
    ...
    queryFn: async () => {
      const res = await supabase.from("producer_stock").select("*");
      return res.data ?? [];
    },
  });
  ...
  const uploadMutation = useMutation({
    mutationFn: async (payload: typeof form) => {
      ...
      const producerPayload = {
        product_id, product_name: payload.product_name, category: payload.category,
        company_name: payload.company_name, is_perishable: shelfLifeDays>7, shelf_life_days: shelfLifeDays,
        storage_temperature: payload.storage_temperature, lot_id, stock_batch_quantity: qty, quantity_stocked: qty,
        manufacturing_date: payload.manufacturing_date, expiry_date: payload.expiry_date, price_per_unit: price,
        date: new Date().toISOString().slice(0,10),
      };

      const supermarketPayload = {
        product_id, product_name: payload.product_name, category: payload.category,
        company_name: payload.target_supermarket, is_perishable: shelfLifeDays>7, shelf_life_days: shelfLifeDays,
        storage_temperature: payload.storage_temperature, lot_id, quantity: qty,
        manufacturing_date: payload.manufacturing_date, expiry_date: payload.expiry_date, price_per_unit: price,
        source_producer: payload.company_name, transfer_date: new Date().toISOString(), date: new Date().toISOString().slice(0,10),
      };
    ...
    onSuccess: () => {
      ...
      setForm({
        ...
        company_name: "", target_supermarket: SUPERMARKETS[0],
      });
    },
    onError: (err:any) => {
      ...
      <Card className="border-success/20">
        <CardHeader className="bg-success/5">
          <CardTitle className="flex items-center gap-2 text-success">
            <Upload className="h-5 w-5" /> Upload & Dispatch Stock
          </CardTitle>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={form.target_supermarket} onValueChange={(v)=>updateField("target_supermarket", v)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{SUPERMARKETS.map(s=> <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
              </Select>
            </div>

            <div className="col-span-1 md:col-span-2">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">Auto shelf life: <strong>{shelfLifeDays} days</strong></div>
              ...
              <div className="mt-3">
                <Button type="submit" className="w-full" disabled={uploadMutation.status === "pending"}>
                  {uploadMutation.status === "pending" ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Uploading...</> : "Upload & Send"}
                </Button>
              </div>
            </div>
          </form>
      ...
      <Card>
        ...
        <CardContent>
          { producerRefetching && producerStock.length === 0 ? (
            <div className="flex justify-center py-8"><Loader2 className="h-8 w-8 animate-spin text-success" /></div>
